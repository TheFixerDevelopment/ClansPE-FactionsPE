	...
	public function factionChat(PlayerChatEvent $PCE) {
		...
		if($this->plugin->motdWaiting($player)) {
			...
			if(time() - $this->plugin->getMOTDTime($player) > 30) {
				...
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("§bSuccessfully updated the faction description! Type §3/f info §6to see your faction info.", true));
	...
	public function factionBlockPlaceProtect(BlockPlaceEvent $event) {
		...
		if($this->plugin->isInPlot($event->getPlayer())) {
			...
			if($this->plugin->inOwnPlot($event->getPlayer())) {
				...
				$event->getPlayer()->sendMessage($this->plugin->formatMessage("You cannot place blocks here. This is already a property of a faction. Type /f plotinfo or /f map for details."));
